...
<div class="admin-wrapper">
    ...
    <main class="admin-content">
        <div class="content-header">
            <h4><i class="bi bi-upload"></i> 上传照片</h4>
            <a href="${pageContext.request.contextPath}/admin/photo" class="btn btn-outline-light btn-sm"><i class="bi bi-arrow-left"></i> 返回</a>
        </div>
        <div class="glass-card">
            <form action="${pageContext.request.contextPath}/admin/photo?action=upload" method="post" enctype="multipart/form-data">
                <div class="mb-3"><label class="form-label">选择照片</label><input type="file" class="form-control glass-input" name="photo" accept="image/*" required></div>
                <div class="mb-3">
                    <label class="form-label">相册名称</label>
                    <input type="text" class="form-control glass-input" name="albumName" list="albumList" placeholder="默认相册">
                    <datalist id="albumList"><c:forEach items="${albums}" var="a"><option value="${a}"></c:forEach></datalist>
                </div>
                <div class="mb-3"><label class="form-label">描述</label><input type="text" class="form-control glass-input" name="description"></div>
